<?php
/**
 * Promotions Carousel — Canyon location
 * Pulls promotions assigned to the "Canyon" category/taxonomy term.
 *
 * ─── TAXONOMY SETUP ────────────────────────────────────────────────────────
...
$promo_canyon_query = new WP_Query( [
	'post_type'      => 'promotion',
	'post_status'    => 'publish',
	'posts_per_page' => -1,
	'orderby'        => 'date',
	...
	'tax_query'      => [
		[
			...
			'field'    => 'slug',
			'terms'    => 'canyon',   // ← update to your exact term slug
		],
	],
] );

if ( ! $promo_canyon_query->have_posts() ) {
	wp_reset_postdata();
	return;
}
...
while ( $promo_canyon_query->have_posts() ) {
	$promo_canyon_query->the_post();
...
if ( empty( $promo_canyon_cards ) ) return;

usort( $promo_canyon_cards, fn( $a, $b ) => $a['priority'] <=> $b['priority'] );

// Show arrows only if more than one card exists
$show_arrows = count( $promo_canyon_cards ) > 1;

?>

<section class="cbv-carousel cbv-carousel--promotions" aria-label="canyon Promotions">
	<div class="cbv-carousel__wrapper">
		<?php if ( $show_arrows ) : ?>
		<button class="cbv-carousel__arrow cbv-carousel__arrow--prev" aria-label="Previous promotions" hidden>
		...
		<div class="cbv-carousel__track" role="list">
			<?php foreach ( $promo_canyon_cards as $card ) : ?>
				<article class="cbv-card" role="listitem">
					...
					<div class="cbv-card__wave">
						...
						<div class="cbv-card__content">
							...
							<?php if ( $card['price'] ) : ?>
								<div class="cbv-card__price">
									<?php echo esc_html( $card['price'] ); ?>
								</div>
							<?php endif; ?>

							<div class="cbv-card__dt-row" aria-label="<?php echo $card['has_date'] ? 'End date' : 'Duration'; ?>">
		...
		<?php if ( $show_arrows ) : ?>
		<button class="cbv-carousel__arrow cbv-carousel__arrow--next" aria-label="Next promotions" hidden>
			<svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
