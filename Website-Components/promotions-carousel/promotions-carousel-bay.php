<?php
/**
 * Promotions Carousel — Bay location
 * Pulls promotions assigned to the "Bay" category/taxonomy term.
 *
 * ─── TAXONOMY SETUP ────────────────────────────────────────────────────────
...
$promo_bay_query = new WP_Query( [
	...
	'tax_query'      => [
		[
			...
			'field'    => 'slug',
			'terms'    => 'bay',   // ← update to your exact term slug
		],
	],
] );

if ( ! $promo_bay_query->have_posts() ) {
	wp_reset_postdata();
	return;
}
...
while ( $promo_bay_query->have_posts() ) {
	$promo_bay_query->the_post();
...
if ( empty( $promo_bay_cards ) ) return;

usort( $promo_bay_cards, fn( $a, $b ) => $a['priority'] <=> $b['priority'] );

// Show arrows only if more than one card exists
$show_arrows = count( $promo_bay_cards ) > 1;

?>

<section class="cbv-carousel cbv-carousel--promotions" aria-label="bay Promotions">
	<div class="cbv-carousel__wrapper">
		...
		<div class="cbv-carousel__track" role="list">
			<?php foreach ( $promo_bay_cards as $card ) : ?>
