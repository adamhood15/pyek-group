<?php
/**
 * Events Carousel — Austin location
 * CPT: events | Taxonomy: category | Term: austin
 * Enqueue: events-carousel.css + events-carousel.js
 */

$today = new DateTime( 'today' );

$events_austin_query = new WP_Query( [
	'post_type'      => 'events',
	'post_status'    => 'publish',
	'posts_per_page' => -1,
	'orderby'        => 'date',
...
if ( ! $events_austin_query->have_posts() ) {
	wp_reset_postdata();
	return;
}
...
while ( $events_austin_query->have_posts() ) {
	$events_austin_query->the_post();
...
if ( empty( $austin_cards ) ) return;

usort( $austin_cards, function ( $a, $b ) {
...
$show_arrows = count( $austin_cards ) > 1;

?>

<section class="tt-carousel tt-carousel--events" aria-label="Austin Upcoming Events">
	<div class="tt-carousel__wrapper">
		...
		<div class="tt-carousel__track" role="list">
			<?php foreach ( $austin_cards as $card ) : ?>
