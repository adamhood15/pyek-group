<?php
/**
 * Events Carousel — Houston location
 * CPT: events | Taxonomy: category | Term: houston
 * Enqueue: events-carousel.css + events-carousel.js
 */

$today = new DateTime( 'today' );

$events_houston_query = new WP_Query( [
	'post_type'      => 'events',
	'post_status'    => 'publish',
	'posts_per_page' => -1,
	'orderby'        => 'date',
...
if ( ! $events_houston_query->have_posts() ) {
	wp_reset_postdata();
	return;
}
...
while ( $events_houston_query->have_posts() ) {
	$events_houston_query->the_post();
...
if ( empty( $houston_cards ) ) return;

usort( $houston_cards, function ( $a, $b ) {
...
$show_arrows = count( $houston_cards ) > 1;

?>

<section class="tt-carousel tt-carousel--events" aria-label="Houston Upcoming Events">
	<div class="tt-carousel__wrapper">
		...
		<div class="tt-carousel__track" role="list">
			<?php foreach ( $houston_cards as $card ) : ?>
				<article class="tt-card" role="listitem">
					...
					<div class="tt-card__wave">
						...
						<div class="tt-card__content">
							...
							<?php $date_display = $card['date'] ?: $card['fallback_msg']; ?>
							<?php if ( $date_display ) : ?>
								<div class="tt-card__dt-row" aria-label="Date">
									...
									<span class="tt-card__date"><?php echo esc_html( $date_display ); ?></span>
								</div>
							<?php endif; ?>
