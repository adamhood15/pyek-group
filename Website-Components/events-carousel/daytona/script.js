  /* DL Carousel — arrow navigation + dots + infinite loop
 * Handles all .dl-carousel sections on the page (events and promotions).
...
(function () {
  ...
  function initCarousel(section) {
    var track   = section.querySelector('.dl-carousel__track');
    var btnPrev = section.querySelector('.dl-carousel__arrow--prev');
    var btnNext = section.querySelector('.dl-carousel__arrow--next');
    var dotsEl  = section.querySelector('.dl-carousel__dots');
    ...
    function buildDots() {
      ...
      dotsEl.innerHTML = '';
      dots = [];
      var cards = Array.from(track.querySelectorAll('.dl-card'));
      cards.forEach(function (card, i) {
        var dot = document.createElement('button');
        dot.className = 'dl-carousel__dot';
    ...
    function updateDots() {
      ...
      var cards = track.querySelectorAll('.dl-card');
      var active = 0;
      ...
      dots.forEach(function (dot, i) {
        dot.classList.toggle('dl-carousel__dot--active', i === active);
      });
    }
  ...
  function init() {
    document.querySelectorAll('.dl-carousel').forEach(initCarousel);
  }
