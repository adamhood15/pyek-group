<?php
/**
 * Events Carousel — Canyon location
 * CPT: events | Taxonomy: category | Term: canyon
 * Enqueue: events-carousel.css + events-carousel.js
 */

$today = new DateTime( 'today' );

$events_canyon_query = new WP_Query( [
	'post_type'      => 'events',
	'post_status'    => 'publish',
	'posts_per_page' => -1,
	'orderby'        => 'date',
...
if ( ! $events_canyon_query->have_posts() ) {
	wp_reset_postdata();
	return;
}
...
while ( $events_canyon_query->have_posts() ) {
	$events_canyon_query->the_post();
	...
	$fallback_msg = trim( (string) ( $ts['date_fallback_message'] ?? '' ) );

	$canyon_cards[] = [
		'title'    => get_field( 'event_name' ) ?: get_the_title(),
		'img_url'  => $img_url,
		'img_alt'  => $img_alt,
		'date'     => $fallback_msg ?: $date_data['display'],
...
if ( empty( $canyon_cards ) ) return;

usort( $canyon_cards, function ( $a, $b ) {
...
$show_arrows = count( $canyon_cards ) > 1;

?>

<section class="cbv-carousel cbv-carousel--events" aria-label="canyon Upcoming Events">
	<div class="cbv-carousel__wrapper">
		<?php if ( $show_arrows ) : ?>
		<button class="cbv-carousel__arrow cbv-carousel__arrow--prev" aria-label="Previous events" hidden>
		...
		<div class="cbv-carousel__track" role="list">
			<?php foreach ( $canyon_cards as $card ) : ?>
				<article class="cbv-card" role="listitem">

					<?php if ( $card['img_url'] ) : ?>
						<div class="cbv-card__image-wrap">
							<img
								class="cbv-card__image"
					...
					<div class="cbv-card__wave">
						<svg class="cbv-card__wave-shape" viewBox="0 0 400 36" preserveAspectRatio="none" aria-hidden="true">
							<path d="M0,18 C80,36 160,0 240,18 C320,36 380,10 400,18 L400,36 L0,36 Z" fill="white"/>
						</svg>

						<div class="cbv-card__content">
							<h3 class="cbv-card__title">
								<?php echo esc_html( $card['title'] ); ?>
							</h3>

							<?php if ( $card['date'] ) : ?>
								<div class="cbv-card__dt-row" aria-label="Date">
									<svg class="cbv-card__icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
										<rect x="3" y="4" width="18" height="18" rx="2"/>
										<path d="M16 2v4M8 2v4M3 10h18"/>
										<path d="M9 16l2 2 4-4"/>
									</svg>
									<span class="cbv-card__date"><?php echo esc_html( $card['date'] ); ?></span>
								</div>
							<?php endif; ?>

							<?php if ( $card['time'] ) : ?>
								<div class="cbv-card__dt-row" aria-label="Time">
									<svg class="cbv-card__icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
										<circle cx="12" cy="12" r="10"/>
										<path d="M12 6v6l4 2"/>
									</svg>
									<span class="cbv-card__time"><?php echo esc_html( $card['time'] ); ?></span>
								</div>
							<?php endif; ?>

							<a
								class="cbv-card__button"
		 ...
		 <?php if ( $show_arrows ) : ?>
		<button class="cbv-carousel__arrow cbv-carousel__arrow--next" aria-label="Next events" hidden>
			<svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
