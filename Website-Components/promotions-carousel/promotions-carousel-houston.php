<?php
/**
 * Promotions Carousel — Houston location
 * Pulls promotions assigned to the "Houston" category/taxonomy term.
 *
 * ─── TAXONOMY SETUP ────────────────────────────────────────────────────────
...
$promo_houston_query = new WP_Query( [
	'post_type'      => 'promotion',
	'post_status'    => 'publish',
	'posts_per_page' => -1,
	'orderby'        => 'date',
	...
	'tax_query'      => [
		[
			...
			'field'    => 'slug',
			'terms'    => 'houston',  // ← update to your exact term slug
		],
	],
] );

if ( ! $promo_houston_query->have_posts() ) {
	wp_reset_postdata();
	return;
}
...
while ( $promo_houston_query->have_posts() ) {
	$promo_houston_query->the_post();
...
if ( empty( $promo_houston_cards ) ) return;

usort( $promo_houston_cards, fn( $a, $b ) => $a['priority'] <=> $b['priority'] );

// Show arrows only if more than one card exists
$show_arrows = count( $promo_houston_cards ) > 1;

?>

<section class="tt-carousel tt-carousel--promotions" aria-label="Houston Promotions">
	<div class="tt-carousel__wrapper">
		 <?php if ( $show_arrows ) : ?>
		<button class="tt-carousel__arrow tt-carousel__arrow--prev" aria-label="Previous promotions" >
		...
		<div class="tt-carousel__track" role="list">
			<?php foreach ( $promo_houston_cards as $card ) : ?>
