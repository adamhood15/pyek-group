<?php
/**
 * Events Carousel — Bay location
 * CPT: events | Taxonomy: category | Term: bay
 * Enqueue: events-carousel.css + events-carousel.js
 */

$today = new DateTime( 'today' );

$events_bay_query = new WP_Query( [
	'post_type'      => 'events',
	'post_status'    => 'publish',
	'posts_per_page' => -1,
	'orderby'        => 'date',
...
if ( ! $events_bay_query->have_posts() ) {
	wp_reset_postdata();
	return;
}
...
while ( $events_bay_query->have_posts() ) {
	$events_bay_query->the_post();
...
if ( empty( $bay_cards ) ) return;

usort( $bay_cards, function ( $a, $b ) {
...
$show_arrows = count( $bay_cards ) > 1;

?>

<section class="cbv-carousel cbv-carousel--events" aria-label="bay Upcoming Events">
